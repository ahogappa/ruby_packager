  ...
  if File.directory?(expand_path)
    Async do
      Find.find(expand_path) do |path|
        Find.prune if path.end_with?('.git') || path.end_with?('/ports') || path.end_with?('/logs') || path.end_with?('/spec') || path.end_with?('.github') || path.end_with?('/docs') || path.end_with?('/exe')
        Find.prune if path.end_with?('3.3.0')
